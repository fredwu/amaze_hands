RSpec.describe Producers::StandardDeviationProducer do
  ...
  describe '#apply' do
    ...
    context 'wait times' do
      ...
      its(:wait_time) do
        is_expected.to eq(
          '2015-4' => {
            combined: { item_values: [1.0], sum: 1.0, count: 1, mean: 1.0, median: 1.0, standard_deviation: 0.0 }
          },
          '2015-5' => {
            combined: { item_values: [2.0, 3.0], sum: 5.0, count: 2, mean: 2.5, median: 2.5, standard_deviation: 0.71 }
          }
        )
      end
    end
